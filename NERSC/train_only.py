...
if __name__ == '__main__':
    ...
    images = data['images'][:args.nb_train_events]
    labels = data['labels'][:args.nb_train_events]
    weights = data['weights'][:args.nb_train_events]
    val = h5py.File(args.val_data)
    images_val = val['images_val'][:args.nb_test_events]
    labels_val = val['labels_val'][:args.nb_test_events]
    weights_val = val['weights_val'][:args.nb_test_events]  
    ...
    if args.model == 'BDT':
       ...
       # print 'Validation accuracy:', score[1]
        ...
        model.compile(
            optimizer='adam',
            loss='binary_crossentropy',
            metrics=['accuracy',tf.keras.metrics.FalsePositives(name='false_positives'),tf.keras.metrics.FalseNegatives(name='false_negatives'),tf.keras.metrics.TrueNegatives(name='true_negatives'),tf.keras.metrics.TruePositives(name='true_positives')]
		)
